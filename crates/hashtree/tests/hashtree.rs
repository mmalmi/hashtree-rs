...
mod stream_builder {
    ...
    async fn test_stream_builder_basic() {
        let (_, tree) = make_tree_with_chunk_size(100);

        let mut stream = tree.stream_builder();
    ...
    #[tokio::test]
    async fn test_stream_builder_checkpoints() {
        let (_, tree) = make_tree_with_chunk_size(100);
    ...
    #[tokio::test]
    async fn test_stream_builder_empty() {
        let (store, tree) = make_tree();

        let stream = tree.stream_builder();
        let (hash, size) = stream.finalize().await.unwrap();
