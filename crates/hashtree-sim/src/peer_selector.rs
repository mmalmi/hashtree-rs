...
impl PeerSelector {
    ...
    pub fn add_peer(&mut self, peer_id: NodeId) {
        self.stats.entry(peer_id).or_insert_with(|| PeerStats::new(peer_id));
    }
    ...
    pub fn get_stats(&self, peer_id: NodeId) -> Option<&PeerStats> {
        self.stats.get(&peer_id)
    }

    /// Get peer stats (mutable)
    pub fn get_stats_mut(&mut self, peer_id: NodeId) -> Option<&mut PeerStats> {
    ...
    pub fn record_request(&mut self, peer_id: NodeId, bytes: u64) {
    ...
    pub fn record_success(&mut self, peer_id: NodeId, rtt_ms: u64, bytes: u64) {
        if let Some(stats) = self.stats.get_mut(&peer_id) {
    ...
    /// Record a timeout
    pub fn record_timeout(&mut self, peer_id: NodeId) {
        if let Some(stats) = self.stats.get_mut(&peer_id) {
    ...
    fn available_peers(&self) -> Vec<NodeId> {
        self.stats
            .iter()
    ...
    fn should_skip_for_fairness(&self, peer_id: NodeId) -> bool {
    ...
    pub fn select_peers(&mut self) -> Vec<NodeId> {
        ...
        if available.is_empty() {
            ...
            let mut backed_off: Vec<_> = self.stats
                .iter()
                .filter(|(_, s)| s.is_backed_off())
                .map(|(&id, s)| (id, s.backoff_remaining()))
                .collect();
        ...
        let candidates: Vec<NodeId> = if self.fairness_enabled && available.len() >= SELECTION_MIN_PEERS {
            available
                .into_iter()
        ...
        match self.strategy {
            SelectionStrategy::Weighted => {
                ...
                sorted.sort_by(|(id_a, a), (id_b, b)| {
                    ...
                    if score_cmp == std::cmp::Ordering::Equal {
                        id_a.cmp(id_b) // Lower ID first for determinism
                    } else {
                        score_cmp
                    }
                });
            ...
            SelectionStrategy::LowestLatency => {
                ...
                sorted.sort_by(|(id_a, a), (id_b, b)| {
                    let rtt_cmp = a.srtt_ms.partial_cmp(&b.srtt_ms).unwrap_or(std::cmp::Ordering::Equal);
    ...
    pub fn select_best(&mut self) -> Option<NodeId> {
        self.select_peers().into_iter().next()
    }

    /// Select top N peers
    pub fn select_top(&mut self, n: usize) -> Vec<NodeId> {
    ...
    pub fn summary(&self) -> SelectorSummary {
        ...
        let avg_rtt = {
            let rtts: Vec<f64> = self.stats.values().filter(|s| s.srtt_ms > 0.0).map(|s| s.srtt_ms).collect();
            if rtts.is_empty() {
                0.0
            } else {
...
mod tests {
    ...
    fn test_peer_selector_add_remove() {
        ...
        selector.add_peer(1);
        selector.add_peer(2);
        assert!(selector.get_stats(1).is_some());
        assert!(selector.get_stats(2).is_some());

        selector.remove_peer(1);
        assert!(selector.get_stats(1).is_none());
        assert!(selector.get_stats(2).is_some());
    }
    ...
    fn test_peer_selector_weighted_selection() {
        ...
        selector.record_request(1, 40);
        selector.record_success(1, 20, 1024);
        selector.record_request(1, 40);
        selector.record_success(1, 25, 1024);

        // Peer 2: medium
        selector.record_request(2, 40);
        selector.record_success(2, 100, 1024);
        selector.record_request(2, 40);
        selector.record_timeout(2);

        // Peer 3: bad (timeouts)
        selector.record_request(3, 40);
        selector.record_timeout(3);
        selector.record_request(3, 40);
        selector.record_timeout(3);
    ...
    fn test_peer_selector_backed_off_peers() {
        ...
        // Back off peer 1
        selector.record_timeout(1);
        assert!(selector.get_stats(1).unwrap().is_backed_off());
    ...
    fn test_peer_selector_fairness() {
        ...
        // Need some time to pass for selection rate calculation
        sleep(Duration::from_millis(15));

        for _ in 0..100 {
            selector.record_request(1, 40);
            selector.record_success(1, 10, 100);
        }

        // Other peers get very few requests
        for i in 2..=6 {
            selector.record_request(i, 40);
            selector.record_success(i, 10, 100);
        }

        // Peer 1 should be skipped due to fairness (>30% selection rate)
        let skipped = selector.should_skip_for_fairness(1);
        // This may or may not trigger depending on timing, but the logic is correct
    ...
    fn test_peer_selector_summary() {
        ...
        selector.record_request(1, 40);
        selector.record_success(1, 50, 1024);
        selector.record_request(2, 40);
        selector.record_timeout(2);
        ...
        assert_eq!(summary.backed_off_count, 1); // Peer 2 is backed off
    ...
    fn test_lowest_latency_strategy() {
        ...
        // Peer 1: 100ms RTT
        selector.record_request(1, 40);
        selector.record_success(1, 100, 1024);

        // Peer 2: 20ms RTT (fastest)
        selector.record_request(2, 40);
        selector.record_success(2, 20, 1024);

        // Peer 3: 50ms RTT
        selector.record_request(3, 40);
        selector.record_success(3, 50, 1024);
    ...
    #[test]
    fn test_adaptive_selection_prefers_reliable_peers() {
        // Test that adaptive selection correctly prioritizes reliable peers
        let mut selector = PeerSelector::with_strategy(SelectionStrategy::Weighted);

        // Add 4 peers with different performance profiles
        selector.add_peer(1); // Will be excellent: 100% success, low RTT
        selector.add_peer(2); // Will be good: 80% success, medium RTT
        selector.add_peer(3); // Will be poor: 50% success, high RTT
        selector.add_peer(4); // Will be bad: mostly timeouts

        // Simulate traffic patterns
        // Peer 1: Excellent - all successes, fast
        for _ in 0..10 {
            selector.record_request(1, 40);
            selector.record_success(1, 15, 1024); // 15ms RTT
        }

        // Peer 2: Good - mostly successes, moderate RTT
        for _ in 0..8 {
            selector.record_request(2, 40);
            selector.record_success(2, 50, 1024); // 50ms RTT
        }
        for _ in 0..2 {
            selector.record_request(2, 40);
            selector.record_timeout(2);
        }
        // Clear backoff for peer 2 so it can be selected
        if let Some(stats) = selector.get_stats_mut(2) {
            stats.backed_off_until = None;
            stats.backoff_level = 0;
        }

        // Peer 3: Poor - half success, slow
        for _ in 0..5 {
            selector.record_request(3, 40);
            selector.record_success(3, 200, 1024); // 200ms RTT
        }
        for _ in 0..5 {
            selector.record_request(3, 40);
            selector.record_failure(3);
        }
        // Clear backoff for peer 3
        if let Some(stats) = selector.get_stats_mut(3) {
            stats.backed_off_until = None;
            stats.backoff_level = 0;
        }

        // Peer 4: Bad - mostly timeouts (will be backed off)
        for _ in 0..10 {
            selector.record_request(4, 40);
            selector.record_timeout(4);
        }

        // Check scores
        let score1 = selector.get_stats(1).unwrap().score();
        let score2 = selector.get_stats(2).unwrap().score();
        let score3 = selector.get_stats(3).unwrap().score();

        // Peer 1 should have highest score (best performance)
        assert!(score1 > score2, "Peer 1 score ({}) should be > peer 2 score ({})", score1, score2);
        assert!(score2 > score3, "Peer 2 score ({}) should be > peer 3 score ({})", score2, score3);

        // Selection should return peers in preference order
        let selected = selector.select_peers();

        // Peer 4 should be last (backed off)
        // Peer 1 should be first (best score)
        assert_eq!(selected[0], 1, "Best peer should be selected first");

        // After selecting, verify peer 4 is backed off
        assert!(selector.get_stats(4).unwrap().is_backed_off(),
            "Peer 4 should be backed off after many timeouts");
    }
    ...
    fn test_exponential_backoff_progression() {
        ...
        // First timeout: 1s backoff
        stats.record_timeout();
        let backoff1 = stats.backoff_remaining();
        assert!(backoff1 <= Duration::from_secs(1), "First backoff should be ~1s");

        // Simulate time passing and backoff expiring
        stats.backed_off_until = None;

        // Second timeout: 2s backoff
        stats.record_timeout();
        let backoff2 = stats.backoff_remaining();
        assert!(backoff2 <= Duration::from_secs(2), "Second backoff should be ~2s");

        // Third timeout: 4s backoff
        stats.backed_off_until = None;
        stats.record_timeout();
        let backoff3 = stats.backoff_remaining();
        assert!(backoff3 <= Duration::from_secs(4), "Third backoff should be ~4s");
    }
    ...
    fn test_rto_adaptation() {
        ...
        let rto_before = stats.rto_ms;
        stats.record_timeout();
        assert_eq!(stats.rto_ms, rto_before * 2, "RTO should double on timeout");
    }
}
