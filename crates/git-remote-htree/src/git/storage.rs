...
use hashtree_core::{sha256, HashTree, HashTreeConfig, DirEntry, Store, Cid};
...
use super::object::{GitObject, ObjectId, ObjectType, parse_tree};
use super::refs::{validate_ref_name, NamedRef, Ref};
...
mod tests {
    ...
    fn test_working_tree_extraction() {
        use crate::git::object::serialize_tree;
        ...
        let src_tree_content = serialize_tree(&[
            crate::git::object::TreeEntry::new(0o100644, "main.rs".to_string(), main_rs_oid),
        ]);
        let src_tree_oid = storage.write_tree(&src_tree_content).unwrap();

        // 3. Create root tree
        let root_tree_content = serialize_tree(&[
            crate::git::object::TreeEntry::new(0o100644, "README.md".to_string(), readme_oid),
            crate::git::object::TreeEntry::new(0o40000, "src".to_string(), src_tree_oid),
        ]);
    ...
    fn test_full_push_simulation() {
        ...
        let src_tree = serialize_tree(&[
            crate::git::object::TreeEntry::new(0o100644, "lib.rs".to_string(), blob_oids[2]),
            crate::git::object::TreeEntry::new(0o100644, "main.rs".to_string(), blob_oids[3]),
        ]);
        let src_tree_oid = storage.write_tree(&src_tree).unwrap();

        // Create root tree
        let root_tree = serialize_tree(&[
            crate::git::object::TreeEntry::new(0o100644, "Cargo.toml".to_string(), blob_oids[1]),
            crate::git::object::TreeEntry::new(0o100644, "README.md".to_string(), blob_oids[0]),
            crate::git::object::TreeEntry::new(0o40000, "src".to_string(), src_tree_oid),
        ]);
