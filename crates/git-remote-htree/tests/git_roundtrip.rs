...
//!
//! **Prerequisites**: The `git-remote-htree` binary must be installed/built:
//!   cargo install --path crates/git-remote-htree --force
//! Or for debug builds:
...
"#);
    ...
    fn update_blossom_servers(&self, servers: &[&str], relay_url: &str) {
        let config_dir = self.home_dir.join(".hashtree");
...
fn test_server_coverage_full_upload() {
    ...
    // Start local relay
    let relay = test_relay::TestRelay::new(19210);
    println!("Started local nostr relay at: {}", relay.url());
...
fn test_non_fast_forward_rejected() {
    ...
    // Start local servers
    let relay = test_relay::TestRelay::new(19220);
